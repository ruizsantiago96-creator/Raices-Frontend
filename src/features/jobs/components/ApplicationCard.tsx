...
export interface ApplicationCardProps {
  app: JobApplication
  onMessage: (job: JobApplication) => void
}
...
export default function ApplicationCard({ app, onMessage }: ApplicationCardProps) {
  const [expanded, setExpanded] = useState(false)

  const statusKey = (app.status ?? 'pending') as keyof typeof STATUS_COLORS
  const badgeColor = STATUS_COLORS[statusKey] ?? '#888'
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 15, color: 'var(--fg1)', marginBottom: 2, fontFamily: 'var(--font-display)' }}>
            {app.title}
          </div>
          <div style={{ fontSize: 13, color: 'var(--fg3)' }}>
            {app.institution_name} · {app.modality}
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          {app.institution_owner_id && (
            <button
              onClick={(e: MouseEvent) => { e.stopPropagation(); onMessage(app) }}
              style={{
      ...
      {expanded && (
        ...
        >
          {app.description && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--fg3)', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 6 }}>
                {JOBS_UI.DESCRIPTION_LABEL}
              </div>
