 ...
 * - click_card: 2 puntos
 */
export function registrarInteraccion(institucionId: string | number, tipo: string, categoria?: string) {
...
export function useRegistrarInteraccion() {
  ...
  return useMutation({
    mutationFn: ({ institucionId, tipo, categoria }: { institucionId: string | number; tipo: string; categoria: string }) =>
