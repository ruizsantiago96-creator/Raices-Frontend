...
export default function EditarInstitucionPage() {
  ...
  const [form, setForm] = useState<EditarFormState>({
    nombre: '',
    descripcion: '',
    categoria: '',
    subcategoria: '',
  ...
  const [csfResult, setCsfResult] = useState<ValidarCsfQrResponse | null>(null)
  ...
  const removeServicio = (srv: string) => {
  ...
  const handleCsfUpload = async () => {
    ...
    try {
      ...
      const msg = errorObj.response?.data?.message || errorObj.response?.data?.error || 'Error al validar la CSF'
      addToast(msg, 'error')
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      ) as UpdateInstitucionPayload

      await updateInst.mutateAsync(datosLimpios)
      setSuccessMsg('Institución actualizada correctamente.')
    } catch (err: unknown) {
      const errorObj = err as { response?: { data?: { message?: string; error?: string } } }
      const msg = errorObj.response?.data?.message || errorObj.response?.data?.error || 'Error al actualizar la institución.'
      setApiError(msg)
    }
  }
  ...
  return (
    <main id="main" className="responsive-main" style={{ '--main-max-width': '800px' } as Record<string, string>}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            {form.urlLogo && (
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                <img src={form.urlLogo} alt="Logo preview" style={{ width: 48, height: 48, borderRadius: 8, objectFit: 'cover', border: '1px solid var(--border-color)' }} onError={e => (e.currentTarget.style.display = 'none')} />
          ...
          <div style={{ marginBottom: 0 }}>
            ...
            {form.urlPortada && (
              <div style={{ marginTop: 8 }}>
                <img src={form.urlPortada} alt="Cover preview" style={{ width: '100%', maxHeight: 120, borderRadius: 8, objectFit: 'cover', border: '1px solid var(--border-color)' }} onError={e => (e.currentTarget.style.display = 'none')} />
              </div>
            )}
          </div>
        </div>
