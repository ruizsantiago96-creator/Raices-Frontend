...
export default function ReviewActions({ review, institutionId }: ReviewActionsProps) {
  const [editing, setEditing] = useState(false)
  const [rating, setRating] = useState(review.rating)
  const [comment, setComment] = useState(review.comment ?? '')
  const updateReview = useUpdateReview(String(review.id), String(institutionId))
  const deleteReview = useDeleteReview(String(review.id), String(institutionId))
  ...
  if (editing) {
    return (
      <div style={{ marginTop: 12, paddingLeft: 46 }}>
        <StarRow rating={rating} size={20} interactive onPick={(n: number) => setRating(n)} />
