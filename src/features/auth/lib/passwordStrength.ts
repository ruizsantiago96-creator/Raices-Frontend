...
export interface PasswordCriterionResult extends PasswordCriterion {
  met: boolean
}
...
export function checkPasswordCriteria(password = ''): {
  criteria: PasswordCriterionResult[]
  missing: PasswordCriterionResult[]
  isValid: boolean
} {
  const criteria: PasswordCriterionResult[] = PASSWORD_CRITERIA.map(item => ({
    ...item,
