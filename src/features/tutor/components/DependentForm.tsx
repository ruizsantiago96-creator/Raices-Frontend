...
export default function DependentForm({ initial, onCancel, onSave, saving = false, relationships = [], disabilities = [] }: {
  initial?: Dependiente | null
  onCancel: () => void
  onSave: (data: CrearDependientePayload | UpdateDependentPayload) => void
  saving?: boolean
  relationships?: string[]
  lifeStages?: EtapaVidaOption[]
  disabilities?: string[]
}) {
  ...
  const [form, setForm] = useState<{
    ...
    parentesco: initial?.parentesco ?? ((initial as Record<string, unknown> | undefined)?.relacion as string | undefined) ?? relationships[0] ?? '',
    tiposDiscapacidad: initial?.tiposDiscapacidad ?? ((initial as Record<string, unknown> | undefined)?.discapacidades as string[] | undefined) ?? [],
  ...
  const toggleDis = (d: string) => setForm(f => ({ ...f, tiposDiscapacidad: f.tiposDiscapacidad.includes(d) ? f.tiposDiscapacidad.filter(x => x !== d) : [...f.tiposDiscapacidad, d] }))
  
  const submit = (e: FormEvent) => {
    e.preventDefault()
    if (!form.nombreCompleto.trim()) return
    if (initial?.id) {
      onSave(form as unknown as Parameters<typeof onSave>[0])
    } else {
      const f = form as unknown as {
        nombreCompleto: string; parentesco: string; tiposDiscapacidad: string[];
        etapaVida: string | null; birth_date: string | null; notas: string;
        crearCuenta?: boolean; email?: string; password?: string;
      }
      const payload: CrearDependientePayload = {
        nombreCompleto: f.nombreCompleto.trim(), parentesco: f.parentesco,
        necesidades: f.tiposDiscapacidad, etapaVida: f.etapaVida || undefined,
        birth_date: f.birth_date || undefined, notas: f.notas || '',
      }
      if (f.crearCuenta && f.email?.trim() && f.password) {
        payload.crearCuenta = true; payload.email = f.email.trim(); payload.password = f.password
      }
      onSave(payload as unknown as Parameters<typeof onSave>[0])
    }
  }
