...
function DependienteCard({ dep }: { dep: Dependiente }) {
  ...
  const discapacidades: string[] = Array.isArray(rawDiscapacidades) ? (rawDiscapacidades as string[]) : []

  const initials = nombre.split(' ').map((w: string) => w?.[0]).filter(Boolean).join('').toUpperCase().slice(0, 2)
  const notas = typeof dep?.notas === 'string' ? dep.notas : typeof dep?.notes === 'string' ? (dep.notes as string) : undefined
    ...
    <div className="card" style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {discapacidades.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {discapacidades.map((d: string, i: number) => (
