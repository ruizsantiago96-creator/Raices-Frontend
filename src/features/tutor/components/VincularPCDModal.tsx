...
export interface VincularPCDModalProps {
  onVincular: (email: string) => void
  onCancel: () => void
  saving?: boolean
}

export default function VincularPCDModal({ onVincular, onCancel, saving = false }: VincularPCDModalProps) {
  const [pcdEmail, setPcdEmail] = useState('')
  const isValidEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(pcdEmail)
  const submit = (e: FormEvent) => { e.preventDefault(); if (!isValidEmail) return; onVincular(pcdEmail.trim()) }
