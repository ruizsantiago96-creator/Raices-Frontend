...
import type { Dependiente, EtapaVidaOption } from '@/types/tutor'
...
/**
 * @param {{ dep: Dependiente, lifeStages?: EtapaVidaOption[], isLinked?: boolean, onEdit?: () => void, onDelete?: () => void, onUnlink?: () => void, onConfigureFeatures?: () => void, onPermissions?: (data: { id: string | number; nombreCompleto?: string }) => void, activeMenuId?: string | number | null, setActiveMenuId?: (id: string | number | null) => void }} props
 */
export default function DependentCard({ dep, lifeStages = [], isLinked = false, onEdit, onDelete, onUnlink, onConfigureFeatures, onPermissions, activeMenuId, setActiveMenuId }: {
  ...
  onPermissions?: (data: { id: string | number; nombreCompleto?: string }) => void
  ...
  const stage = lifeStages.find((l: EtapaVidaOption) => l.id === dep?.etapaVida)
  ...
  const handleAIRefresh = (e: React.MouseEvent) => {
    e?.stopPropagation?.()
    aiRec.mutate(dep?.id)
  ...
  const handleMenuClick = (e: React.MouseEvent) => {
    e.stopPropagation()
    setActiveMenuId?.(isMenuOpen ? null : (dep?.id ?? dep?.pcdUserId ?? null))
  }
  const hasPhoto = typeof dep?.fotoUrl === 'string' && dep.fotoUrl.length > 0
  ...
  return (
    <div className="card" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        {hasPhoto ? (
          <img src={dep.fotoUrl as string} alt={nombre} aria-hidden="true" style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
        ) : (
        ...
        <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {aiRec.isPending ? (
            ...
            <>
              ...
              {normalized && normalized.steps && normalized.steps.length > 0 ? (
