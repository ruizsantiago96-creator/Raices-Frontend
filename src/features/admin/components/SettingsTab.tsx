...
export default function SettingsTab() {
  ...
  const current: Record<string, string | number | boolean> = form ?? (settings as unknown as Record<string, string | number | boolean>) ?? {}
  const set = (k: string, v: string | number | boolean) => setForm({ ...current, [k]: v })
          ...
          {SETTING_FIELDS.map(f => (
            <div key={f.key} className="admin-settings-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, paddingBottom: 14, borderBottom: '1px solid var(--border-color)' }}>
              ...
              {f.type === 'toggle' ? (
                ...
                <input type={f.type} value={String(current[f.key] ?? '')} onChange={e => set(f.key, e.target.value)}
