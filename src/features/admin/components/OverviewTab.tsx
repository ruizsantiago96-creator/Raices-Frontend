...
interface AnalyticsItem {
  ...
  users?: number | string
  instituciones?: number | string
  institutions?: number | string
  resenas?: number | string
  reviews?: number | string
  publicaciones?: number | string
  posts?: number | string
}

interface ActiveUsersDetail {
  live?: number
  promedioDiario?: number
  promedioSemanal?: number
  promedioMensual?: number
  historialMinutos?: number[]
}

export default function OverviewTab({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  const { data: stats, isLoading } = useAdminStats() as { data: AdminStats | undefined; isLoading: boolean }
  const { data: rawAnalytics, isLoading: isAnalyticsLoading } = useAdminDetailedAnalytics() as { data: AnalyticsItem[] | undefined; isLoading: boolean }
  const { data: activeUsersDetail, isLoading: isActiveUsersLoading } = useAdminActiveUsersDetail() as { data: ActiveUsersDetail | undefined; isLoading: boolean }

  const c1 = 'var(--chart-1, var(--primary))'
  const c2 = 'var(--chart-2, color-mix(in oklch, var(--primary) 65%, var(--secondary)))'
  const c3 = 'var(--chart-3, color-mix(in oklch, var(--secondary) 40%, black))'
  const c4 = 'var(--chart-4, color-mix(in oklch, var(--secondary) 40%, white))'
  ...
  const statCards: { label: string; value: number | undefined; sub: string; icon: (props: { s?: number }) => React.ReactNode; color: string }[] = [
  ...
  const parseAnalytics = (): { label: string; usuarios: number; instituciones: number; resenas: number; publicaciones: number }[] => {
    if (!rawAnalytics) return []

    let arr = null as AnalyticsItem[] | null
    ...
    const val = (item: AnalyticsItem, m: string) => {
      ...
      for (const f of fields) {
        const key = item[f as keyof AnalyticsItem]
        if (key != null) {
          return typeof key === 'number' ? key : parseInt(String(key), 10) || 0
        }
      }
      return 0
    }
  ...
  const chartPoints: { label: string; usuarios: number; instituciones: number; resenas: number; publicaciones: number }[] = parseAnalytics()
  ...
  const pts: { x: number; y: number }[] = rawPts.map((val: number, idx: number) => {
    const liveCount = (activeUsersDetail?.live ?? stats?.usuariosActivos ?? 0)
          ...
          <div key={c.label} className={`admin-kpi-card animate-fade-in-up delay-${idx + 1}`}
            ...
            <div style={{ minWidth: 0 }}>
              <div className="kpi-value" style={{ fontSize: 20, fontWeight: 800, fontFamily: 'var(--font-display)', color: 'var(--fg1)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
                {isLoading ? <Skeleton w={40} h={20} /> : <AnimatedCounter value={c.value as number} />}
              </div>
            ...
            <div style={{ flex: 1, position: 'relative', height: '100%' }}>
              ...
              {isAnalyticsLoading ? (
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'end', gap: 16, padding: '0 8px', justifyContent: 'space-around' }}>
                  {displayPoints.map((p: { label: string; usuarios: number; instituciones: number; resenas: number; publicaciones: number }, idx: number) => {
        ...
        <Card className="animate-fade-in-up delay-5" style={{ padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            {[{ k: 'usuarios', c: c1, l: 'Usuarios' }, { k: 'instituciones', c: c2, l: 'Instituciones' }, { k: 'resenas', c: c3, l: 'Reseñas' }, { k: 'publicaciones', c: c4, l: 'Publicaciones' }].map(item => (
              <div
                key={item.k}
                onMouseEnter={() => setHoveredDonut(item.k as string)}
