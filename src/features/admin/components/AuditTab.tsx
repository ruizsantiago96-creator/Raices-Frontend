...
function getAccionStyle(accion: string | undefined | null) {
...
export default function AuditTab({ onNavigate }: AuditTabProps = {}) {
  const [filters, setFilters] = useState<{ pagina: number; limite: number; accion?: string; recurso?: string }>({ pagina: 1, limite: 20 })
  const [filterAcción, setFilterAccion] = useState('')
  const [filterRecurso, setFilterRecurso] = useState('')

  const queryParams: { pagina: number; limite: number; accion?: string; recurso?: string } = { ...filters }
  ...
  const logsRaw = (auditData as { datos?: unknown[]; data?: unknown[]; total?: number; totalPaginas?: number } | undefined)?.datos ?? (auditData as { datos?: unknown[]; data?: unknown[]; total?: number; totalPaginas?: number } | undefined)?.data ?? (Array.isArray(auditData) ? auditData as unknown[] : [])
  const logs = logsRaw as { id?: string | number; accion?: string; descripcion?: string; recurso?: string; usuarioNombre?: string; fechaCreacion?: string }[]
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      {isLoading ? (
        ...
        <div style={{
          ...
          {logs.map((log: { id?: string | number; accion?: string; descripcion?: string; recurso?: string; usuarioNombre?: string; fechaCreacion?: string }, i: number) => {
            return <AuditLogRow key={log.id ?? i} log={log} />
          })}
        </div>
      )}
