...
interface InstitutionLike {
  ...
  is_active?: boolean
  is_verified?: boolean
  correos?: (string | { email?: string; correo?: string })[]
  emails?: (string | { email?: string; correo?: string })[]
  correosContacto?: (string | { email?: string; correo?: string })[]
  email?: string
...
function EmailCell({ inst }: { inst: InstitutionLike }) {
...
export default function InstitutionsTab() {
  ...
  const [confirm, setConfirm] = useState<InstitutionLike | null>(null)
  const [editInst, setEditInst] = useState<InstitutionLike | null>(null)
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (actionMenuRef.current && e.target && !(e.target instanceof Node)) { setActionMenuId(null); return }
      if (actionMenuRef.current && e.target && !actionMenuRef.current.contains(e.target as Node)) { setActionMenuId(null) }
    }
    if (actionMenuId) document.addEventListener('mousedown', handler)
    return () => document.removeEventListener('mousedown', handler)
  }, [actionMenuId, menuPos])

  const pendingVerification: InstitutionLike[] = all.filter((inst: InstitutionLike) => !inst.is_verified && inst.is_active)
  const combinedPendingMap = new Map<string | number, InstitutionLike>()
  for (const inst of [...pending, ...pendingVerification]) {
    if (!combinedPendingMap.has(inst.id as string | number)) combinedPendingMap.set(inst.id as string | number, inst)
  }
  const combinedPending: InstitutionLike[] = [...combinedPendingMap.values()]

  const baseRows = filter === 'pending' ? combinedPending : all
  const rows = search.trim()
    ? baseRows.filter((inst: InstitutionLike) => {
  ...
  const totalPages = Math.max(1, Math.ceil((rows as InstitutionLike[]).length / INST_PAGE_SIZE))
  const safePage = Math.min(page, totalPages)
  const paged = (rows as InstitutionLike[]).slice((safePage - 1) * INST_PAGE_SIZE, safePage * INST_PAGE_SIZE) as InstitutionLike[]

  const doApprove = (id: string | number) => approve.mutate(id, { onSuccess: () => addToast('Institución aprobada', 'success') })
  const doVerify = (id: string | number) => verify.mutate(id, { onSuccess: (d: unknown) => { addToast((d as { is_verified?: boolean }).is_verified ? 'Marcada como verificada' : 'Verificación retirada', 'success'); setActionMenuId(null) } })
  const doReject = () => {
    if (!confirm) return
    reject.mutate(confirm.id as string | number, { onSuccess: () => { addToast('Institución eliminada', 'success'); setConfirm(null); setActionMenuId(null) } })
  }
  const doEdit = () => {
    if (!editInst) return
    updateInst.mutate({ id: editInst.id as string | number, ...editForm }, {
      onSuccess: () => { addToast('Institución actualizada', 'success'); setEditInst(null) },
      onError: (e: unknown) => addToast((e as { response?: { data?: { message?: string } } }).response?.data?.message ?? 'Error al actualizar', 'error'),
    })
  }
  const openEdit = (inst: InstitutionLike) => {
    setEditInst(inst)
    setEditForm({ name: inst.name ?? '', email: inst.email ?? '' })
    setActionMenuId(null)
  }

  const inputStyle: React.CSSProperties = { height: 40, padding: '0 12px 0 36px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', fontSize: 14, color: 'var(--fg1)', background: 'var(--bg-surface)', outline: 'none', boxSizing: 'border-box', fontFamily: 'var(--font-body)', width: '100%' }
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', flex: 1, minWidth: 220, marginLeft: 'auto' }}>
          ...
          <input value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} placeholder="Buscar por nombre, ciudad o categoría..."
            style={inputStyle} />
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Icons.check({ s: 32 })} title={filter === 'pending' ? 'Sin instituciones pendientes' : 'No hay instituciones'} sub={filter === 'pending' ? 'Todas las solicitudes han sido procesadas' : undefined} />
      ) : (
        <>
          <div className="responsive-table-wrap" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflowX: 'auto' }}>
            <table className="responsive-table" style={{ width: '100%', minWidth: 750, borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {paged.map((inst: InstitutionLike & { id?: string | number }, i: number) => (
                  <tr key={String(inst.id)} style={{ borderBottom: i < paged.length - 1 ? '1px solid var(--border-color)' : 'none', transition: 'background 0.15s' }}
                    ...
                    <td style={{ padding: '14px 16px', position: 'relative' }}>
                      <button onClick={(e: React.MouseEvent<HTMLButtonElement>) => { if (actionMenuId === inst.id) { setActionMenuId(null); setMenuPos(null) } else { const r = e.currentTarget.getBoundingClientRect();    setMenuPos({ top: r.bottom + 4, right: window.innerWidth - r.right - 16 }); setActionMenuId(inst.id as string | number) } }}
                      ...
                      {actionMenuId === inst.id && menuPos && createPortal(
                        <div ref={actionMenuRef} style={{ position: 'fixed', top: menuPos.top, right: menuPos.right, width: 200, background: 'var(--bg-surface)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-lg)', zIndex: 9999, padding: '6px 0', animation: 'fade-in 0.12s ease-out' }}>
                          ...
                          {!inst.is_active && (
                            <button onClick={() => { doApprove(inst.id as string | number); setActionMenuId(null) }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13.5, fontWeight: 600, color: 'var(--color-artes)', fontFamily: 'var(--font-body)', textAlign: 'left' }}
                              onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-cool)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                              {Icons.check({ s: 15 })} Aprobar
                            </button>
                          )}
                          {inst.is_active && (
                            <button onClick={() => doVerify(inst.id as string | number)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13.5, fontWeight: 600, color: 'var(--primary)', fontFamily: 'var(--font-body)', textAlign: 'left' }}
                              onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-cool)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                          ...
                          <button onClick={() => { if (inst) setConfirm(inst); setActionMenuId(null) }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13.5, fontWeight: 600, color: 'var(--color-error)', fontFamily: 'var(--font-body)', textAlign: 'left' }}
                            onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-cool)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
      ...
      {/* Edit Modal */}
      {editInst && createPortal(
        <div onClick={() => setEditInst(null)} className="modal-overlay" style={{ zIndex: 9999 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', padding: 28, maxWidth: 420, width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <input type="email" value={editForm.email} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditForm({ ...editForm, email: e.target.value })}
                  style={{ width: '100%', height: 40, padding: '0 12px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', fontSize: 14, color: 'var(--fg1)', background: 'var(--bg-surface)', outline: 'none', boxSizing: 'border-box', fontFamily: 'var(--font-body)' }} />
              </div>
            </div>
